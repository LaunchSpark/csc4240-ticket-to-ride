...
- `applications/`: first-party UI surfaces. The app surface is the marimo notebook harness in `applications/notebook_harness/` — a pure-Python package (no HTTP/PocketBase dependency) that bot notebooks under `integrations/external/bots/` import to run and render in-process test games. It's registered in `pyproject.toml`'s wheel `packages` and installed as a top-level `notebook_harness` package (imported as `from notebook_harness... import ...`, not `applications.notebook_harness`) — required so it resolves via site-packages regardless of how a notebook file is opened (marimo's kernel process does not inherit the repo-root-relative `sys.path` that `uv run`/`pytest`-style invocation sets up).
